...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--crop_ratio', type=float, default=1, help='Crop ratio')
        ...
        parser.add_argument('--dataset_mode', type=str, default='facades')
        ...
        parser.add_argument('--normalize', action='store_true', help='Normalize input/output')
        parser.add_argument('--imagenet_norm', action='store_true', help="Uses imagenet mean and std")
        ...
        parser.add_argument('--netG', type=str, default='ConditionedUnet', help='selects model to use for netG', choices=["ConditionedUnet", "ASAPNets", "Pix2PixHD"])
        ...
        parser.add_argument('--image_format', type=str, default=".jpg", help='Format for storing images')
        ...
        ## For unet and ConditionedUnet
        parser.add_argument('--tiles', type=int, default=0, help='Trains the model in a tiled manner. 0 means regular training. -1 means mixture of :[1,4, 32, 64]', choices=[-1, 0, 1, 4, 8, 9, 16, 64])
        parser.add_argument('--tileSize', type=int, default=None, help='Specifies a tile Size for Training or Inference')
        parser.add_argument('--condition', type=str, default=None, help='Uses Spatially Adaptive Denormalization Layers in different ways', choices=["mixed", "preprocess", "full-preprocess", "same", "none"])
        parser.add_argument('--unet_depth', type=int, default=3, help='Downscaling steps in unet')
        parser.add_argument('--base_channels', type=int, default=64, help='Base proyection channels')
        parser.add_argument('--global_base_channels', type=int, default=16, help='Base proyection channels for global path')
        parser.add_argument('--condition_hidden_channels', type=int, default=64, help='Hidden channels for condition layers')
        parser.add_argument('--res_blocks', type=int, default=4, help='Residual blocks in the neck of the unet')
        parser.add_argument('--proyection_kernel', type=int, default=3, help='Kernel for proyection layers')
        parser.add_argument('--slope', type=float, default=None, help='Slope of the Leaky ReLU, None for ReLU')
        parser.add_argument('--norm_groups', type=int, default=None, help='Groups for group norm')
        parser.add_argument('--padding_size', type=int, default=2, help='Reflection padding to apply ')
        parser.add_argument('--num_stages', type=int, default=3, help='Number of stages in HRNet')
        parser.add_argument('--stage_layers', type=int, default=2, help='Number of layers for stage in HRNet')
        parser.add_argument('--decoder_factor', type=int, default=1, help='Decoder size factor compared to encoder')
        parser.add_argument('--upsample_type', type=str, default="bilinear", help='Interpolation mode for upsampling', choices=["nearest", "bilinear", "bicubic"])
        parser.add_argument('--interpolation', type=str, default="bilinear", help='Interpolation mode for upsampling', choices=["nearest", "bilinear", "bicubic"])
        parser.add_argument('--skip_type', type=str, default="noskip", help='Mode for skip connections mode for upsampling', choices=["residual", "dense", "noskip"])
