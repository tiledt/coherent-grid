...
class FacadesDataset(Pix2pixDataset):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        #parser.set_defaults(dataroot=os.path.expanduser('~') + '/data/facadesHR')
        parser.set_defaults(dataroot='./dataset/facadesHR')
        parser.set_defaults(preprocess_mode='resize_and_crop')
        load_size = 286 if is_train else 256
        parser.set_defaults(load_size=load_size)
        parser.set_defaults(display_freq=318 * 2)
        parser.set_defaults(print_freq=318)
    ...
    def get_paths(self, opt):
        root = opt.dataroot
        phase = 'val' if opt.phase == 'test' or opt.phase == 'val' else opt.phase
