...
class Pix2pixDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        # Label Image or Input image
        label_path = self.label_paths[index]
        label = Image.open(label_path)
        #label.save('%s.jpg'%label_path[:-4],'JPEG')
        params = get_params(self.opt, label.size)
        
        transforms_list = get_transform(self.opt, params,  method=InterpolationMode.BICUBIC)
        if self.opt.no_one_hot:
            label = label.convert('RGB')
            transform_image = transforms.Compose(transforms_list)
            label_tensor = transform_image(label)
        else:
            #transform_label = transforms.Compose(transforms_list)
            transform_label = transforms.Compose(transforms_list[:-1]) # without normalization
        ...
        # keep same transformation for target
